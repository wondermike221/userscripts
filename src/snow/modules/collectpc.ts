...
export default async function scrapeCollectPC() {
  ...
  const info_btn: HTMLElement = document.querySelector(
    '#viewr\\.sc_task\\.request_item\\.request\\.requested_for',
  );
  await info_btn.click();
  //get NT, QID, Date
  const elem_nt: HTMLInputElement = document.querySelector(
    '#sys_user\\.u_configuration_item_label',
  );
  fields.NT = elem_nt.value;
  const elem_qid: HTMLInputElement = document.querySelector(
    '#sys_readonly\\.sys_user\\.x_ebay_core_config_sam_qid',
  );
  fields.QID = elem_qid.value;
  const elem_date: HTMLInputElement = document.querySelector(
