...
const SN_URL_REGEX = /\/record\/(sc_task|sc_req_item|sys_user|alm_hardware|incident)\/([a-f0-9]{32})/i;
...
function createAndInvokeMailto(to, cc, subject, body) {
...
function sendEmailFromTemplate(template) {
  ...
  createAndInvokeMailto(template.to, template.cc, template.subject, template.body);
  ...
  if (template.htmlBody) {
    copyHtmlToClipboard(template.htmlBody).then(success => {
      if (success) {
        ...
        showNotification('Rich HTML version copied to clipboard - paste into your email client for better formatting!');
      }
    });
  }
}
...
function makeTrackingLink(tracking) {
  let trackingLink = '';
  ...
  if (/^1Z[0-9A-Z]{16}$/.test(tracking)) {
    ...
    courierName = 'UPS';
  } else if (/^[0-9]{12}$/.test(tracking) || /^[0-9]{15}$/.test(tracking) || /^[0-9]{20}$/.test(tracking)) {
    // FedEx tracking number (12, 15, or 20 digits)
    trackingLink = `https://www.fedex.com/fedextrack/?trknbr=${tracking}`;
    courierName = 'FedEx';
  } else if (/^(94|93|92|94|95)[0-9]{20}$/.test(tracking) || /^[A-Z]{2}[0-9]{9}US$/.test(tracking)) {
...
const s = {
  p: text => `<p style="margin:0 0 12px 0;font-family:Calibri,sans-serif;font-size:11pt;">${text}</p>`,
  li: text => `<li style="font-family:Calibri,sans-serif;font-size:11pt;margin-bottom:4px;">${text}</li>`,
  ul: items => `<ul style="margin:8px 0 12px 0;padding-left:20px;">${items.map(s.li).join('')}</ul>`,
  b: text => `<strong>${text}</strong>`,
  a: (url, text) => `<a href="${url}" style="color:#0563C1;">${text}</a>`,
  br: () => '<br>'
};

// Wraps parts in an Outlook-safe container div
function emailBody(...parts) {
...
async function getIncident(sysId) {
  return getSnowRecord('incident', sysId);
}
...
async function fetchTicketData() {
  ...
  if (table === 'incident') {
    ...
    if (!incident) {
      ui.showToast('Failed to load incident', {
        theme: 'dark'
      });
      return null;
    }
...
function copyJson(data) {
  ...
  const [,, html, json] = build_charge_sheet_row_cis(task, user);
...
function copyDropship(data) {
  copyRichTextToClipboard(build_bh_sheet_row_cis(data.task, data.user));
  ui.showToast('Dropship row copied to clipboard', {
    theme: 'dark'
  });
}
function copyExit(data) {
  const {
    task,
    user,
    manager,
  ...
  if (!manager) {
    ui.showToast('No manager found for user', {
      theme: 'dark'
    });
    return;
  }
  const asset = assets.filter(a => task.u_variables_parsed.v_assets_to_return.includes(a.asset_tag));
  copyRichTextToClipboard(build_exit_sheet_row_cis(task, user, manager, asset[0]));
  ui.showToast('Exit row copied to clipboard', {
    theme: 'dark'
  });
}
...
const ShadowQuery = {
  ...
  find(selector, root = document) {
    ...
    const findRecursive = currentRoot => {
  ...
  findAll(selector, root = document) {
    ...
    const findAllRecursive = currentRoot => {
...
function fillField(selector, text, label) {
  const el = ShadowQuery.find(selector);
  if (!el) {
    ui.showToast(`${label} field not found`, {
      theme: 'dark'
    });
    return;
  }
  ...
  ui.showToast(`${label} filled`, {
    theme: 'dark'
  });
}
function fillWorkNotes(text) {
  fillField('textarea[name*="work_notes"]', text, 'Work notes');
}

function yubikeyTemplate(data, tracking) {
  var _user$dv_name, _user$dv_email;
  const {
    task,
    user
  } = data;
  const firstName = ((_user$dv_name = user.dv_name) != null ? _user$dv_name : '').split(' ')[0];
...
Regards,
ITSS Team`;
  const htmlBody = emailBody(s.p(`Hi ${s.b(firstName)},`), s.p('Your YubiKey has been shipped!'), s.p(`${s.b('Tracking:')} ${tracking}`), s.p('Please let us know once you receive it.'), s.p('Regards,<br>ITSS Team'));
  return {
...
const yubikeyWorkflow = {
  label: 'YubiKey Ticket',
  load: async () => {
    const data = await fetchTicketData();
    if (!data || data.type !== 'sc_task') return errorNode('Open an SC task first');
    return buildSteps([['getJson', step.action('1. Copy JSON (for label making)', () => copyJson(data))], ['tracking', step.input('2. Tracking Number', 'text', 'yubikeyTracking')], ['complete', step.action('3. Fill Notes + Send Email', () => {
      var _localStorage$getItem;
      const tracking = (_localStorage$getItem = localStorage.getItem('yubikeyTracking')) != null ? _localStorage$getItem : '';
...
function accessoryDeliveryTemplate(data, tracking) {
  var _user$dv_name, _user$dv_email;
  const {
    task,
    user
...
Thank you,
ITSS Global Support`;
  const htmlBody = emailBody(s.p(`${s.b('Great news ' + firstName + '!')} Your accessories are on their way.`), s.p(`${s.b('Tracking:')} ${makeTrackingLink(tracking)}`), s.p(`Please be ready to sign for delivery. We'll be closing your request now.`), s.p('Thank you,<br>ITSS Global Support'));
  return {
    to: (_user$dv_email = user.dv_email) != null ? _user$dv_email : '',
    cc: 'servicenow@ebay.com',
...
function accessoryDeliveredTemplate(data) {
  var _user$dv_name2, _user$dv_email2;
  const {
    task,
    user
  } = data;
...
Regards,
ITSS Team`;
  const htmlBody = emailBody(s.p(`Hi ${s.b(firstName)} –`), s.p('Our records indicate your equipment was delivered successfully, your equipment request ticket has been closed.'), s.p('Regards,<br>ITSS Team'));
  return {
    to: (_user$dv_email2 = user.dv_email) != null ? _user$dv_email2 : '',
    cc: 'servicenow@ebay.com',
...
const accessoryDropshipWorkflow = {
  label: 'Accessory Dropship',
  load: async () => {
    const data = await fetchTicketData();
    if (!data || data.type !== 'sc_task') return errorNode('Open an SC task first');
    return buildSteps([['getJson', step.action('1. Copy JSON (for label making)', () => copyJson(data))], ['sheet', step.action('2. Copy Dropship Sheet Row', () => copyDropship(data))], ['tracking', step.input('3. Tracking Number', 'text', 'accessoryTracking')], ['complete', step.action('4. Fill Notes + Send Shipping Email', () => {
      var _localStorage$getItem;
      const tracking = (_localStorage$getItem = localStorage.getItem('accessoryTracking')) != null ? _localStorage$getItem : '';
      fillWorkNotes(`Shipped via FedEx. Tracking: ${tracking}`);
      sendEmailFromTemplate(accessoryDeliveryTemplate(data, tracking));
    })]]);
  }
};
const accessoryDeliveredWorkflow = {
  label: 'Accessory Delivered',
  load: async () => {
    const data = await fetchTicketData();
    if (!data || data.type !== 'sc_task') return errorNode('Open an SC task first');
    return buildSteps([['send', step.action('Send Delivered Email', () => {
      sendEmailFromTemplate(accessoryDeliveredTemplate(data));
    })]]);
  }
};

function assetsFromData(data) {
  var _data$task$dv_u_varia;
  const taskUVars = JSON.parse((_data$task$dv_u_varia = data.task.dv_u_variables) != null ? _data$task$dv_u_varia : '{}');
  return data.assets.filter(a => {
    var _taskUVars$v_assets_t;
    return (_taskUVars$v_assets_t = taskUVars.v_assets_to_return) == null ? void 0 : _taskUVars$v_assets_t.includes(a.asset_tag);
  }).map(a => {
    var _a$dv_model, _a$dv_serial_number, _a$dv_asset_tag;
    return {
      model: (_a$dv_model = a.dv_model) != null ? _a$dv_model : '',
      serialNumber: (_a$dv_serial_number = a.dv_serial_number) != null ? _a$dv_serial_number : '',
      assetTag: (_a$dv_asset_tag = a.dv_asset_tag) != null ? _a$dv_asset_tag : ''
    };
  });
}
function workdayFirstStrikeTemplate(data) {
  var _JSON$parse$contact_e, _data$task$dv_u_varia2, _user$dv_name, _data$manager$dv_emai, _data$manager;
  const {
    task,
    user
  } = data;
  ...
  const contactEmail = (_JSON$parse$contact_e = JSON.parse((_data$task$dv_u_varia2 = data.task.dv_u_variables) != null ? _data$task$dv_u_varia2 : '{}').contact_email) != null ? _JSON$parse$contact_e : '';
...
Best regards,`;
  const htmlBody = emailBody(s.p(`Dear ${s.b((_user$dv_name = user.dv_name) != null ? _user$dv_name : '')},`), s.p('I hope this message finds you well. I am writing to request the return of company equipment that was assigned to you. Per our records, the following items were issued and have not been returned:'), s.ul(assets.map(a => `${a.model} [SN: ${a.serialNumber}, Tag: ${a.assetTag}]`)), s.p('We kindly ask these items to be returned promptly after leaving the company. A FedEx QR code has been sent to your personal email address on file. This code will allow FedEx to package and ship these items to us free of charge. This email will come directly from FedEx – not eBay. If you do not see it, please check your junk/spam folder.'), s.p(`Thank you for your prompt attention to this matter. If you have any questions, please contact us at ${s.a('mailto:itreturns@ebay.com', 'itreturns@ebay.com')}.`), s.p('Best regards,'));
  return {
    to: contactEmail,
    cc: `${(_data$manager$dv_emai = (_data$manager = data.manager) == null ? void 0 : _data$manager.dv_email) != null ? _data$manager$dv_emai : ''};itreturns@ebay.com;servicenow@ebay.com`,
    subject: `Request for Returned Equipment - ${user.dv_name} | ${task.dv_number}`,
    body: plainBody,
    htmlBody
  };
}
function peopleXFirstStrikeTemplate(data) {
  var _manager$dv_name, _manager$dv_name2, _user$dv_name2, _manager$dv_email;
  const {
    task,
    user,
    manager
  ...
  const assets = assetsFromData(data);
  const formattedAssets = formatAssets(assets);
  const plainBody = `Hi ${(_manager$dv_name = manager == null ? void 0 : manager.dv_name) != null ? _manager$dv_name : 'Manager'},
...
Thanks!`;
  const htmlBody = emailBody(s.p(`Hi ${s.b((_manager$dv_name2 = manager == null ? void 0 : manager.dv_name) != null ? _manager$dv_name2 : 'Manager')},`), s.p(`Your exited employee ${s.b((_user$dv_name2 = user.dv_name) != null ? _user$dv_name2 : '')} has not returned their equipment.`), s.ul(assets.map(a => `${a.model} [SN: ${a.serialNumber}, Tag: ${a.assetTag}]`)), s.p('IT can handle the return but since they were onboarded directly via PeopleX we need your help to get the information necessary to start the process.'), s.p('Please reply with the following information:'), s.ul(['Personal Email', 'Phone Number', 'Address']), s.p('If they have already returned their equipment, please let me know when and where so we may follow up with the local site.'), s.p('Thanks!'));
  return {
...
function fieldglassFirstStrikeTemplate(data) {
  var _JSON$parse$contact_e2, _data$task$dv_u_varia3, _user$dv_name3;
  const {
    task,
    user
  } = data;
  const assets = assetsFromData(data);
  const formattedAssets = formatAssets(assets);
  const contactEmail = (_JSON$parse$contact_e2 = JSON.parse((_data$task$dv_u_varia3 = data.task.dv_u_variables) != null ? _data$task$dv_u_varia3 : '{}').contact_email) != null ? _JSON$parse$contact_e2 : '';
...
Unreturned Devices:
${formattedAssets}`;
  const htmlBody = emailBody(s.p('Hi <point of contact>,'), s.p(`We are looking for the return information for the device(s) listed below from an exited member of your team ${s.b((_user$dv_name3 = user.dv_name) != null ? _user$dv_name3 : '')}. Please provide the following information so we can ensure proper processing for this asset return.`), s.ul(['Carrier:', 'Tracking Number:', 'eBay office being shipped to:']), s.p('As a reminder we ask for all eBay items to be returned within 10 days.'), s.p('Thank you for your cooperation,'), s.p(`${s.b('Unreturned Devices:')}`), s.ul(assets.map(a => `${a.model} [SN: ${a.serialNumber}, Tag: ${a.assetTag}]`)));
  return {
    to: contactEmail,
    cc: 'servicenow@ebay.com',
...
const exitWorkflow = {
  ...
  load: async () => {
    const data = await fetchTicketData();
    if (!data || data.type !== 'sc_task') return errorNode('Open an SC task first');
    return buildSteps([['sheet', step.action('1. Copy Exit Sheet Row', () => copyExit(data))]]);
  }
};
const firstStrikeWorkflow = {
  label: 'First Strike Email',
  load: async () => {
    const data = await fetchTicketData();
    if (!data || data.type !== 'sc_task') return errorNode('Open an SC task first');
    const kbs = [['kbProcess', step.kb('KB: Exit Process', 'https://placeholder.example.com/exit-process')], ['kbFedex', step.kb('KB: FedEx Return Labels', 'https://placeholder.example.com/fedex-returns')]];
    const source = data.user.dv_u_worker_source;
    const emailStep = source === 'Workday' ? ['send', step.action('Send Workday Template', () => sendEmailFromTemplate(workdayFirstStrikeTemplate(data)))] : source === 'PeopleX' ? ['send', step.action('Send PeopleX Template', () => sendEmailFromTemplate(peopleXFirstStrikeTemplate(data)))] : source === 'Fieldglass' ? ['send', step.action('Send Fieldglass Template', () => sendEmailFromTemplate(fieldglassFirstStrikeTemplate(data)))] : ['chooseTemplate', step.branch('Choose Template', async () => ['Workday', 'PeopleX', 'Fieldglass'], value => {
      if (value === 'Workday') sendEmailFromTemplate(workdayFirstStrikeTemplate(data));else if (value === 'PeopleX') sendEmailFromTemplate(peopleXFirstStrikeTemplate(data));else if (value === 'Fieldglass') sendEmailFromTemplate(fieldglassFirstStrikeTemplate(data));
    })];
    return buildSteps([...kbs, emailStep]);
  }
};
const secondStrikeWorkflow = {
      ...
      config: {
        ...
        children: {
          techNT: {
            ...
            inputType: 'text',
            storageKey: 'techNT',
            onChange: value => ui.showToast(`Tech NT set to: ${value}`, {
              theme: 'dark'
            })
          }
        }
