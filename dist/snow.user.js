...
function openEmail(user, subject, body = '') {
  var _user$dv_email;
  createAndInvokeMailto((_user$dv_email = user.dv_email) != null ? _user$dv_email : '', 'servicenow@ebay.com', subject, body);
  ui.showToast('Email draft opened', {
    theme: 'dark'
  });
}
...
async function copyCrosscharge() {
  ...
  const tsv = [new Date().toISOString(), 'SLC', '', '1', task.dv_number, user.dv_email, user.dv_cost_center].join('\t');
  const html = convertPlainTextToHTMLTable(tsv);
  const json = {
  ...
  ui.showToast('CrossCharge row copied to clipboard', {
    theme: 'dark'
  });
}
async function copyChargesheet() {
  ...
  const [cis] = build_charge_sheet_row_cis(task, user);
  copyRichTextToClipboard(cis);
  ui.showToast('Chargesheet row copied to clipboard', {
    theme: 'dark'
  });
}
async function copyDropship() {
  ...
  copyRichTextToClipboard(build_bh_sheet_row_cis(task, user));
