...
function initializeAutofill() {
  ...
  waitForElm(signature_selector).then(() => {
    const signature_checkbox = document.querySelector(signature_selector);
    signature_checkbox == null || signature_checkbox.click();
    const changeEvent = new Event('change');
  ...
  waitForElm(signature_option_selector).then(() => {
    const signature_option = document.querySelector(signature_option_selector);
  ...
  waitForElm(shipping_account_selector).then(() => {
    const shipping_account = document.querySelector(shipping_account_selector);
  ...
  function autoFillAction(e) {
    ...
    for (const field in FORM_FIELDS) {
      ...
      if (FORM_FIELDS[field].type === 'dropdown-text') {
        ...
        selectByText(input, FORM_FIELDS[field].value || ship[field]);
      } else {
